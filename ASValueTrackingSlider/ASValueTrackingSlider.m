...
- (void)setup
{
    ...
    self.popUpView = [[ASValuePopUpView alloc] initWithFrame:CGRectMake(0, 0, 60, 30)];//CGRectZero (0, 0, 50, 30)
...
//    [self.popUpView setString:[_numberFormatter stringFromNumber:@(self.value)]];//原来的
//    [self.popUpView setAnimationOffset:[self currentValueOffset]];
//    [self autoColorTrack];
    
    
    [self adjustPopUpViewFrame];
    [self.popUpView setString:_myValueString];
    [self.popUpView setAnimationOffset:[self currentValueOffset]];
    [self autoColorTrack];
...
- (void)calculatePopUpViewSize
{
    ...
    _popUpViewSize = [self.popUpView popUpSizeForString:[NSString stringWithFormat:@"期望年化收益率 %@",[_numberFormatter stringFromNumber:@(value)]]];
}
