...
//    _navTabBar = [[SCNavTabBar alloc] initWithFrame:CGRectMake(DOT_COORDINATE, DOT_COORDINATE, SCREEN_WIDTH, NAV_TAB_BAR_HEIGHT) showArrowButton:_showArrowButton];
    if(_barFrame.size.width <= 0 && _barFrame.size.height <=0){
         _navTabBar = [[SCNavTabBar alloc] initWithFrame:CGRectMake(DOT_COORDINATE, DOT_COORDINATE, SCREEN_WIDTH, NAV_TAB_BAR_HEIGHT) showArrowButton:_showArrowButton];
    }else{
        _navTabBar = [[SCNavTabBar alloc] initWithFrame:_barFrame showArrowButton:_showArrowButton];
    ...
    if (_isHaveBarFlag) {
        _mainView = [[UIScrollView alloc] initWithFrame:CGRectMake(DOT_COORDINATE, _navTabBar.frame.origin.y + _navTabBar.frame.size.height, SCREEN_WIDTH, SCREEN_HEIGHT - _navTabBar.frame.origin.y - _navTabBar.frame.size.height-NAVIGATION_BAR_HEIGHT-5)];
    }else{
    ...
    _mainView.scrollEnabled = YES;//不让手动滚动
...
- (void)itemDidSelectedWithIndex:(NSInteger)index
{
    ...
    [_mainView setContentOffset:CGPointMake(index * SCREEN_WIDTH, DOT_COORDINATE) animated:_scrollAnimation];// 这个方法回引起 scrollViewDidScroll
    if (_delegate && [_delegate respondsToSelector:@selector(itemSelectIndex:)]) {
        [_delegate itemSelectIndex:index];
    }
}
