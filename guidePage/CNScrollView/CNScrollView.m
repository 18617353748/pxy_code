...
#pragma mark - 实现代理
-(void)clickItemView{
    if (_delegate && [_delegate respondsToSelector:@selector(clickItemView)]) {
        [_delegate clickItemView];
    }
}
...
-(void)creatItemView{
    ...
    for ( int i = 0; i < _itemsCount; i++ ) {
        ...
        item.cnViewController.title = [NSString stringWithFormat:@"第%d个界面",(int)item.index+1];
...
- (CGFloat)angleForView:(ItemsView *)view
{
    ...
    CGFloat angle = (currentOffsetX - baseOffsetX)/(kScreenW + 80 );//根据比例计算角度 kScreenW-50
