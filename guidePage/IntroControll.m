...
- (id)initWithFrame:(CGRect)frame pages:(NSArray*)pagesArray
{
    ...
    if(self != nil) {
        ...
        for(int i = 0; i <  pages.count; i++) {
            ...
            [view addTarget:self action:@selector(blockme:) forControlEvents:UIControlEventTouchUpInside];
...
- (void) initShow {
    ...
    if(scrollPhotoNumber != currentPhotoNum) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(currentIndex:)]) {
            [_delegate currentIndex:currentPhotoNum];
        }
    }
    
    float offset =  scrollView.contentOffset.x - (currentPhotoNum * self.frame.size.width);
    //NSLog(@"::: %f",scrollView.contentOffset.x);
    if (_delegate && [_delegate respondsToSelector:@selector(currentOffset:)]) {
        [_delegate currentOffset:scrollView.contentOffset.x];
    }
